/* ThreePhaseCloud class written by Sean Fraser, based on
Kyle McDonald's PhaseWrap and PhaseUnwrap functions.

Class contains all of the values needed to calculate and store 
any Structure Light pixel clouds */


class ThreePhaseCloud {
  // ------------------------------------------ //
  //                  PROPERTIES
  // ------------------------------------------ //
  public int mySet;
  ...
  //Constructor 1
  public ThreePhaseCloud (int setNum, float zsc,float zsk, float noises) {
    mySet = setNum;
    zscale = zsc;
    zskew =zsk;
  ...
  //BY SEAN
  color[][] duplicateColorArray(){
    color[][] result = new color[inputHeight][inputWidth];
    for (int y = 0; y < inputHeight; y++) {
      for (int x = 0; x < inputWidth; x++) {
  ...
  float[][] duplicateDepthArray(){
    float[][] result = new float[inputHeight][inputWidth];
    for (int y = 0; y < inputHeight; y++) {
      for (int x = 0; x < inputWidth; x++) {
        result[y][x] = depth[y][x];
      }
    }
    return result;
  }
